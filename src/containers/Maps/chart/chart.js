...
export const CreateChartAPI = () => {
  ...
  const createChart = (node, data,data1, data2) => {
    ...
    yscale_sum = d3.scaleLinear()
    .domain([setup.dataRangeMin, 3*setup.dataRangeMax])
    .range([height, 0]);
  ...
  const envelope_=(data,operatorColor, data1, operatorColor1, data2, operatorColor2)=>{
    ...
    .attr("transform", `translate(${margin.left+xscale.bandwidth()/2-xscale.bandwidth()+space},${margin.top})`)
