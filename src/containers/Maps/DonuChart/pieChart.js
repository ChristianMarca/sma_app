...
import { interpolate,nest} from "d3";
import {tsv} from 'd3-request';
import  './style.css';
//import {data as data_} from './species.js';
const d3={select,scaleOrdinal,interpolate};
...
export const createDonutChart=()=>{
  ...
  var i = 0;
  function donutChart_(id_node,data){
    donut.data(data);
    d3.select(id_node)
        .call(donut); // draw chart in div
    // tsv('species.tsv', function(error,data) {
    //    console.log('la data',data,error)
    //    if (error) {console.log('aqui el error',error);throw error;}
...
const donutChart=()=>{
    var data = [],
        ...
        colour = scaleOrdinal(d3.schemeCategory20c), // colour scheme
    ...
    function chart(selection){
        selection.each(function() {
            ...
            var path = svg.select('.slices')
                ...
                .data(pie_(data))
              .enter().append('path')
                .attr('fill', function(d) {console.log('sad',d); return colour(d.data[category]); })
                .attr('d', arc_);
            ...
            function calculatePoints(d) {
                ...
                return [arc_.centroid(d), outerArc.centroid(d), pos]
            }
            ...
            function labelStyleTween(d) {
                ...
                var interpolate_ = interpolate(this._current, d);
                this._current = interpolate_(0);
                return function(t){
                    var d2 = interpolate_(t);
